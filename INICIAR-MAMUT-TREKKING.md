...
```bash
npx create-next-app@latest mamut-trekking
```
...
```bash
npm run dev      # abre em /pt, testar /en e /es
npm run build    # confirmar que todas as páginas geram estáticas
```
...
- [ ] `/llms.txt` acessível.
- [ ] `npm run build` gera tudo estaticamente, sem erros.
...
> rode `npm run build` e valide o checklist da Parte 7.
