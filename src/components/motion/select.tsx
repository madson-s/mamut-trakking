...
export interface SelectTriggerProps {
  className?: string;
  /** Cor/tamanho do chevron quando o trigger foge do visual padrão de campo. */
  chevronClassName?: string;
  /**
   * `false` desliga o morph de raio (o "gooey") — necessário quando o trigger
   * não é o retângulo de 12px padrão (ex.: pílula), senão os cantos deformam.
   */
  gooey?: boolean;
  /** Nome acessível quando não há rótulo visível associado. */
  'aria-label'?: string;
  children: ReactNode;
}
  ...
  return (
    ...
    >
      ...
      <motion.span
        ...
        className={cn("text-content-muted", chevronClassName)}
      >
...
export function SelectValue({ placeholder, className }: SelectValueProps) {
  ...
  return (
    <span
      className={cn(label ? "text-content" : "text-content-subtle", className)}
    >
...
export interface SelectContentProps {
  className?: string;
  /** Classes do contêiner interno (padding/gap da lista) — padrão `p-1`. */
...
export function SelectContent({ className, innerClassName, children }: SelectContentProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute left-0 right-0 z-20 rounded-control border border-line-strong bg-surface shadow-popover",
...
export interface SelectItemProps {
  value: string;
  disabled?: boolean;
  /** `false` esconde o check do item selecionado (quando a seleção já é marcada de outro jeito). */
  showCheck?: boolean;
  className?: string;
  children: ReactNode;
}
...
export function SelectItem({
  ...
  return (
    <motion.li variants={ctx.reduce ? undefined : ITEM_VARIANTS}>
      ...
      >
        {children}
        {showCheck && selected ? <Check className="h-3.5 w-3.5 shrink-0" /> : null}
      </button>
    </motion.li>
  );
}
